...
func FindProductBuilder(db *sql.DB, req *products.ProductFilter) IFindProductBuilder {
...
func (b *findProductBuilder) Result() []*products.Product {
	...
	rows, err := b.db.Query(b.query, b.values...)
	if err != nil {
	...
	for rows.Next() {
		...
		// if err := rows.Scan(pq.Array(product)); err != nil {
		if err := rows.Scan(&bytes); err != nil {
			log.Printf("scan product failed: %v\n", err)
			return make([]*products.Product, 0)
		}
...
func (b *findProductBuilder) Count() int {
	...
	for rows.Next() {
		if err := rows.Scan(&count); err != nil {
			log.Printf("scan count product failed: %v\n", err)
			return 0
		}
	}
